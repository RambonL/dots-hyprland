# Quickshell II — CLAUDE.md

## Stack

- **Quickshell** — QML-basierter Wayland shell compositor
- **QML / JavaScript** — alle UI-Komponenten
- **Hyprland** — Window Manager
- Shell-Scripts + Python für externe Daten (Batterien, VPN, etc.)
...
Kein `qmldir` nötig — Quickshell auto-discovered alle `.qml` in importierten Verzeichnissen.
...
### Bar-Indikator Pattern (eigener Klick-Handler + Popup)
...
Icons wie Mute, Mic, Bluetooth, Netzwerk sind in `indicatorsRowLayout` in `rightSidebarButton`.  
Pattern: `Revealer { reveal: condition; MaterialSymbol { color: rightSidebarButton.colText } }`  
Diese haben keinen eigenen Klick-Handler (öffnen alle die rechte Sidebar).
...
- **Modifier-Check in Maus-Events**: `onPressed` statt `onClicked` verwenden — Modifier sind beim Drücken zuverlässig vorhanden, nicht beim Loslassen
- **StyledPopup Screen-Clamping**: `StyledPopup.qml` wurde gepatcht um Overflow auf zweiten Monitor zu verhindern (left margin wird auf `screenWidth - popupWidth` geclampt)
